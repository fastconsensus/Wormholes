	...
	for {
		...
		case event, ok := <-c.events.Chan():
			...
			case istanbul.MessageEvent:
				// 如果是普通节点或者不是本轮的validator，直接gossip消息
...
func (c *core) handleTimeoutMsg() {
	...
	if lastProposal != nil && lastProposal.Number().Cmp(c.current.Sequence()) >= 0 {
		...
		miniredis.GetLogCh() <- map[string]string{"handleTimeoutMsg" + lastProposal.Number().String(): "sendNextRoundChange|" +
			" time|" + time.Now().Format("2006-01-02 15:04:05") +
			" no|" + lastProposal.Number().String() +
			" round|" + c.currentView().Round.String(),
		}
