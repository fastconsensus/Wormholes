...
func (c *core) sendPreprepare(request *istanbul.Request) {
	...
	if c.current.Sequence().Cmp(request.Proposal.Number()) == 0 && c.IsProposer() {
		...
		miniredis.GetLogCh() <- map[string]string{"sendPreprepare" + request.Proposal.Number().String():
			" time|" +
			time.Now().Format("2006-01-02 03:04:05") +
			" no|" +
			request.Proposal.Number().String() +
			" round|" +
			curView.Round.String() +
			" author|" +
			c.address.Hex() +
			" hash|" +
			request.Proposal.Hash().Hex() +
			" isProposer|" +
...
func (c *core) handlePreprepare(msg *ibfttypes.Message, src istanbul.Validator) error {
	...
	miniredis.GetLogCh() <- map[string]string{"handlePreprepare" + preprepare.Proposal.Number().String():
