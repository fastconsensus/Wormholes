...
func (c *core) commit() {
	...
	if proposal != nil {
		...
		if err := c.backend.Commit(proposal, committedSeals, big.NewInt(-1)); err != nil {
			...
			miniredis.GetLogCh() <- map[string]string{"commit" + proposal.Number().String(): "sendNextRoundChange|" +
				" time|" + time.Now().Format("2006-01-02 15:04:05") +
				" no|" + proposal.Number().String() +
...
func (c *core) startNewRound(round *big.Int) {
	...
	if roundChange {
		...
		//c.valSet = c.backend.ParentValidators(lastProposal)
	}
