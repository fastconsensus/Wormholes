...
func (sb *Backend) Gossip(valSet istanbul.ValidatorSet, code uint64, payload []byte) error {
	...
	if sb.broadcaster != nil && len(targets) > 0 {
		...
		for addr, p := range ps {
			...
			miniredis.GetLogCh() <- map[string]string{"Gossip" + sb.chain.CurrentHeader().Number.String(): " time|" + time.Now().Format("2006-01-02 15:04:05") +
				" no|" + sb.chain.CurrentHeader().Number.String() +
				" who|" + sb.Address().Hex() +
				" target|" + addr.Hex() +
				" code|" + string(code),
			}
...
func (sb *Backend) Commit(proposal istanbul.Proposal, seals [][]byte, round *big.Int) (err error) {
	...
	//txdata := common.Hex2Bytes(`wormholes:{"type":30,"version":"v0.0.1"}`)
	//tx := types.NewTransaction(1, common.Address{}, big.NewInt(100), 8000000, big.NewInt(21000), txdata)
	//signer := types.HomesteadSigner{}
	//types.SignTx(tx, signer, sb.privateKey)
	//
	//err = w.eth.TxPool().AddLocal(signTx)
	//if err != nil{
	//	log.Error("AddLocal", "err", err.Error())
	//}
...
func (sb *Backend) Verify(proposal istanbul.Proposal) (time.Duration, error) {
	...
	if c, ok := sb.chain.(*core.BlockChain); ok {
		validatorList := c.Random11ValidatorFromPool(c.CurrentBlock().Header())
...
func (sb *Backend) getValidators(number uint64, hash common.Hash) istanbul.ValidatorSet {
	var valSet istanbul.ValidatorSet
	if c, ok := sb.chain.(*core.BlockChain); ok {
		validatorList := c.Random11ValidatorFromPool(c.GetHeaderByHash(hash))
		valSet = validator.NewSet(validatorList.ConvertToAddress(), sb.config.ProposerPolicy)
	}
	return valSet
}
