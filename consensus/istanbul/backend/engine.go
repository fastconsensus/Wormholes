...
func (sb *Backend) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	var valSet istanbul.ValidatorSet
	if c, ok := chain.(*core.BlockChain); ok {
		log.Info("caver|Prepare", "header-no", header.Number.String(), "current-header", c.CurrentBlock().Header().Number.String())
		validatorList := c.Random11ValidatorFromPool(c.CurrentBlock().Header())
		valSet = validator.NewSet(validatorList.ConvertToAddress(), sb.config.ProposerPolicy)
	}
...
func (sb *Backend) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	miniredis.GetLogCh() <- map[string]string{"Seal" + block.Number().String(): " time|" + time.Now().Format("2006-01-02 15:04:05") +
		" no|" + block.Number().String() +
		" src|" + sb.Address().Hex() +
		" hash|" + block.Hash().Hex(),
	}
	...
	if err != nil {
		log.Info("caver|seal|err", "sealNo", block.NumberU64(), "err", err.Error())
		return err
	}
