...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if len(state.PledgedTokenPool) > 0 {
		for _, v := range state.PledgedTokenPool {
			if v.Flag {
				validatorPool.AddValidator(v.Address, v.Amount)
			} else {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	log.Info("caver|insertChain", "len", len(chain))
	...
	for ; block != nil && err == nil || err == ErrKnownBlock; block, err = it.next() {
		...
		activeMiners := bc.ReadActiveMinersPool(parent)
...
func (bc *BlockChain) Random11ValidatorFromPool(header *types.Header) types.ValidatorList {
	activeMiners := bc.ReadActiveMinersPool(header)

	validators := activeMiners.ValidatorByDistanceAndWeight(activeMiners.ConvertToBigInt(), 11, header.Hash())

	var elevenValidator types.ValidatorList
	for _, addr := range validators {
		elevenValidator.AddValidator(addr, activeMiners.StakeBalance(addr))
	}
	return elevenValidator
}

func (bc *BlockChain) ReadActiveMinersPool(header *types.Header) *types.ActiveMinerList {
	activeMiners, _ := rawdb.ReadActiveMinersPool(bc.db, header.Hash(), header.Number.Uint64())
	return activeMiners
}
...
// 确保ActiveMinersPool始终是在validatorPool中
