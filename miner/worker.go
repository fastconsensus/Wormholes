...
func (w *worker) resultLoop() {
	for {
		...
		case block := <-w.resultCh:
			...
			if block.Time()-w.chain.GetHeaderByHash(block.ParentHash()).Time > 20 {
				miniredis.GetLogCh() <- map[string]string{"longTimeOut": " no|" +
					block.Number().String() +
					" mined.time|" +
					time.Unix(int64(block.Time()), 0).Format("2006-01-02 03:04:05") +
					" parent.time|" +
					time.Unix(int64(w.chain.GetHeaderByHash(block.ParentHash()).Time), 0).Format("2006-01-02 03:04:05"),
				}
			}
...
func (w *worker) sendLivenessTx() error {
	...
	if !validatorPool.Exist(w.coinbase) {
		log.Info("caver|sendLivenessTx|not validator", "self", w.coinbase.String())
		return nil
	}

	innerData := `wormholes:{"type":30,"version":"v0.0.1"}`
	txdata := hex.EncodeToString([]byte(innerData))
	txdataByte, _ := hex.DecodeString(txdata)
	tx := types.NewTransaction(w.eth.TxPool().Nonce(w.coinbase), common.Address{}, big.NewInt(0), 50000, big.NewInt(2000000000), txdataByte)
...
func (w *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	if parent.NumberU64() > 0 {
		...
		nominatedOfficialNFT.Dir = "/ipfs/QmYgBEB9CEx356zqJaDd4yjvY92qE276Gh1y2baWeDY3By"
	...
	activeMiners := w.chain.ReadActiveMinersPool(parent.Header())
	state.ActiveMinersPool = activeMiners
	for _, v := range state.ActiveMinersPool.ActiveMiners {
		log.Info("ddddd", "no", header.Number.Uint64(), "state.ActiveMinersPool.ActiveMiners", v)
	}
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		log.Info("caver|worker|commitTransaction", "no", w.current.header.Number.String())
